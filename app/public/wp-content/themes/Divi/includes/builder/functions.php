...
if ( ! defined( 'ET_BUILDER_PRODUCT_VERSION' ) ) {
	...
	define( 'ET_BUILDER_PRODUCT_VERSION', '5.0.0-public-beta.9.1' );
}
...
function et_builder_print_font() {
	...
	// Bail if not inline style, already enqueued, and cached fonts exist.
	if (
		...
		if ( false !== $post_id ) {
			...
			$has_cached_data = is_array( $post_fonts_data )
				&& ! empty( $post_fonts_data['family'] )
				&& ! empty( $post_fonts_data['subset'] );
 ...
 *
 * @since ?? Deprecated. Please use \ET\Builder\Framework\Utility\HTMLUtility::fix_builder_shortcodes() instead.
 *
 * @param string $content content.
 *
...
function et_pb_all_role_options() {
	...
	if ( Conditions::is_d5_enabled() ) {
		...
		foreach ( $all_registered_modules as $module_slug => $module ) {
			...
			// Filter for modules with category 'module'.
			...
			$all_modules_array[] = array(
				...
				'label'              => esc_attr( $module->d4Shortcode ?? $module->name ),
				...
				$output .= sprintf(
					'<div class="et_pb_capability_option">
						...
						<div class="et_pb_yes_no_button_wrapper">
							...
							<select name="%1$s" id="%1$s" class="et-pb-main-setting regular-text">
								<option value="on" %3$s>Yes</option>
								<option value="off" %4$s>No</option>
							</select>
						</div>
					</div>',
					esc_attr( $capability ),
...
function et_fb_add_admin_bar_link() {
	...
	// If the page has a GB Divi Layout Block, clicking "Edit With Divi" will result in a fatal error.
	// To be removed after the Divi Layout Block is fully migrated to D5.
	if ( $post_id && function_exists( 'has_blocks' ) && has_blocks( $post_id ) ) {
		$post = get_post( $post_id );
		if ( $post && has_block( 'divi/layout', $post ) ) {
			return;
		}
	}
