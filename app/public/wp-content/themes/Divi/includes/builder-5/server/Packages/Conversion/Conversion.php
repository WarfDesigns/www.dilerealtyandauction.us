...
class Conversion {
    ...
    public static function valueSanitization($value, $desktopName, $moduleName) {
        ...
        // Sanitize custom CSS so invalid or incomplete declarations (e.g. trailing colon) do not break
        // block comment JSON or parse_blocks() during save-time sanitization (see #47867).
        if (strpos($desktopName, 'custom_css_') === 0) {
            $sanitizedValue = str_replace('||', "\n", $value);
            if (is_string($sanitizedValue)) {
                $sanitizedValue = SavingUtility::sanitize_css_properties($sanitizedValue);
            }
        }
	...
	static function encodeAttrs( array $attrs, string $moduleName, ?string $content = null, $originalShortcode = null, $shortcodeName = null, $postId = null, ?array $parentAttrs = null ): string {
	...
	static function convertShortcodeToGbFormat( $shortcodePart, $gbReset = true, $globalID = null, $postId = null, $isFirstLevel = true, $parentAttrs = null ) {
		static $gbString = '';
		...
		if ( is_array( $element['content'] ) ) {
				...
				self::convertShortcodeToGbFormat( $element['content'], false, $globalModuleID, $postId, false, $child_parent_attrs );
	...
	static function convertAttrs(array $attrs, string $moduleName, ?string $content = null, bool $is_preset_conversion = false): array {
		...
		// For CTA module: if background is disabled and padding doesn't exist, set padding to 0px.
		...
		if ( 'divi/cta' === $moduleName || 'divi/cta' === self::maybe_restore_preset_module_name($moduleName) ) {
			...
			$padding_exists           = isset( $convertedAttrs['module']['decoration']['spacing']['desktop']['value']['padding'] );

			// If use_background_color is explicitly 'off' and padding doesn't exist, set padding to 0px.
			if ( $use_background_color_off && ! $padding_exists ) {
	 ...
	 */
	// phpcs:ignore ET.NamingConventions.VisibilityUnderscore.Private_Method -- Method name follows existing codebase conventions.
