...
class ModuleRegistration {
	...
	/**
	 * Check if module is disabled on all enabled breakpoints.
	 *
	 * @since ??
	 *
	 * @param array $disabled_on_attrs The disabled-on attributes.
	 *
	 * @return bool True if disabled on all enabled breakpoints, false otherwise.
	 */
	private static function _is_disabled_on_all_breakpoints( array $disabled_on_attrs ): bool {
		if ( empty( $disabled_on_attrs ) ) {
			return false;
		}
		...
		$desktop_above_enabled = isset( $disabled_on_attrs['desktopAbove']['value'] ) && 'on' === $disabled_on_attrs['desktopAbove']['value'];
		$tablet_only_enabled   = isset( $disabled_on_attrs['tabletOnly']['value'] ) && 'on' === $disabled_on_attrs['tabletOnly']['value'];
		$phone_enabled         = isset( $disabled_on_attrs['phone']['value'] ) && 'on' === $disabled_on_attrs['phone']['value'];
		...
		foreach ( $enabled_breakpoints as $breakpoint ) {
			...
			// If breakpoint is not in $disabled_on_attrs, treat it as 'off' (not disabled).
			$breakpoint_attr = $disabled_on_attrs[ $breakpoint_name ]['value'] ?? 'off';
			if ( 'on' !== $breakpoint_attr ) {
				return false;
			}
		}
