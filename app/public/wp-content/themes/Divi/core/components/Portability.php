	...
	public function import_layout( $id, $layout, $chunk = 0, $onboarding = false ) {
		...
		if ( $result['ready'] ) {
			...
			foreach ( $post_meta as $entry ) {
				...
				if ( isset( $entry['key'] ) && '_et_pb_custom_css' === $entry['key'] && is_string( $meta_value ) ) {
		...
		foreach ( $data as $key => $value ) {
			...
			if ( is_array( $value ) ) {
				if ( isset( $value['key'], $value['value'] ) && '_et_pb_custom_css' === $value['key'] ) {
					// Custom CSS needs CSS-specific sanitization to preserve escape sequences.
					if ( ! current_user_can( 'unfiltered_html' ) && is_string( $value['value'] ) ) {
						$value['value'] = SavingUtility::sanitize_css( $value['value'] );
					}
...
function et_core_portability_ajax_import() {
	if ( ! isset( $_POST['context'] ) ) {
		...
		$onboarding = isset( $_POST['onboarding'] ) ? '1' === $_POST['onboarding'] : false;
